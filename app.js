...
function showFatalError(err){
  ...
  console.error(err);
  const box = document.querySelector("#fatalError");
  if (box){
    box.style.display = "block";
    box.textContent = "페이지 로딩 중 오류가 발생했어요.\n\n" + msg;
  }
}
...
function setupPasteImport() {
  ...
  btn.addEventListener("click", () => {
    ...
    if (out) out.textContent = `인식: ${result.parsed}줄 / 등록: ${result.added}과목 / 중복·무시: ${result.skipped}`;
    saveState();
    renderAll();
  });
