 ...
 * -------------------------------------------------------- */
// 기대 입력: 포털 성적표/수강내역 표를 그대로 복사하면 보통 탭(TSV)으로 들어옴.
// 예시(줄): 1\t2025\t2학기\tBS119\t02\t...\t일반화학Ⅱ\t...\t3.0\tC0\t2.0
function splitCellsLine(line) {
...
function findCourseCodeFromCells(cells) {
  // BS105a, HSS109a, GC101, BE101a 등
  const codeRe = /^[A-Za-z]{1,6}\d{2,4}[A-Za-z]?$/;
  for (const cell of cells) {
    const t = String(cell).trim();
    if (codeRe.test(t)) return t;
  }
  for (const cell of cells) {
    const m = String(cell).trim().match(/[A-Za-z]{1,6}\d{2,4}[A-Za-z]?/);
    if (m) return m[0];
  }
  return null;
}

function findCreditIdxAndValue(cells) {
  // 학점(0.5~6) 다음에 성적(A0/B+/S/C0...)이 오는 패턴을 우선
  ...
  // fallback: 범위 내 숫자 중 "가장 먼저 나온 값"을 학점으로 취급 (GPA와 헷갈릴 수 있어 보수적으로)
  for (let i = 0; i < cells.length; i++) {
    const v = normalizeNumber(cells[i]);
    if (!Number.isFinite(v) || v < 0.5 || v > 6) continue;
    return { idx: i, credits: v };
  }
  return null;
}

function guessCourseNameFromCells(cells, creditIdx) {
  // creditIdx 이전 셀들 중 "가장 길고 의미 있는" 텍스트를 과목명 후보로
  const end = (typeof creditIdx === "number") ? Math.min(cells.length, creditIdx) : cells.length;

  // 흔한 분류/라벨 단어는 제외
  const stop = new Set([
    "기초", "심화", "교과", "전공", "필수", "선택", "인문사회", "기초과학", "기초공학",
    "글로벌커뮤니케이션", "영어", "수학", "물리", "화학", "생명과학", "컴퓨터공학",
    "쓰기·읽기 중점", "미래소양강좌"
  ]);
...
function importFromPastedTable(text) {
  ...
  for (const line of lines) {
    parsed++;
    const cells = splitCellsLine(line);
    if (cells.length < 3) { skipped++; continue; }

    const code = findCourseCodeFromCells(cells);
    const creditInfo = findCreditIdxAndValue(cells);
    const credits = creditInfo?.credits;

    if (!code || !Number.isFinite(credits) || credits <= 0) { skipped++; continue; }

    // 이미 추가된 과목이면 스킵 (코드 기준)
    const up = code.toUpperCase();
    if (state.courses.some(c => (c.code || "").toUpperCase() === up)) { skipped++; continue; }

    // 과목 데이터(courses.json)에 있으면 그걸 우선 사용(이름/학점 보정 + bucket 자동)
    const catalog = findCourse(code);
    const name = catalog?.name || guessCourseNameFromCells(cells, creditInfo?.idx) || code;

    // credits는 catalog에 있으면 catalog를 우선(정확한 기준), 없으면 포털 복붙값 사용
    const finalCredits = (catalog && Number.isFinite(Number(catalog.credits))) ? Number(catalog.credits) : credits;
...
function setupPasteImportUI() {
  // index.html에 아래 요소가 있으면 자동으로 동작:
  // #pasteBox, #pasteImportBtn, #pasteClearBtn, #pasteResult
  const box = $("#pasteBox");
  const btn = $("#pasteImportBtn");
  const clear = $("#pasteClearBtn");
  const out = $("#pasteResult");

  if (!box || !btn || !clear) return; // UI 없으면 아무 것도 안 함

  btn.addEventListener("click", () => {
    const txt = box.value || "";
    const r = importFromPastedTable(txt);
    if (out) out.textContent = `인식 ${r.parsed}줄 · 추가 ${r.added}과목 · 무시/중복 ${r.skipped}`;
  });
  ...
  // (선택) 붙여넣자마자 자동 분석하고 싶으면 아래 주석 해제
  // box.addEventListener("paste", () => setTimeout(() => btn.click(), 0));
}
