...
function setupPasteImport() {
  ...
  if (!box || !btn) return; // UI not mounted
...
function importFromPastedTable(text) {
  ...
  for (const line of lines) {
    ...
    const bucket = (typeof suggestBucket === "function") ? suggestBucket(code, name) : "adv-other";
...
function findCreditToken(cells) {
  // Credits number usually followed by grade (A0/B+/S/C0...)
...
function guessCourseName(cells, creditIdx) {
  // Heuristic: pick the longest meaningful token before credits
  const end = creditIdx != null ? Math.min(cells.length, creditIdx) : cells.length;
  const stop = new Set(["기초","심화","교과","전공","필수","선택","인문사회","기초과학","기초공학","영어","글로벌커뮤니케이션"]);
